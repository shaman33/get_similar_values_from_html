    ...
    def get_items_from_html(self, html: str):
        ...
        def __find_rich_parent(e, getParent=False):
            ...
            if e.parent == None or e.parent.name == "body":
                return None
            children = e.parent.find_all(e.name, recursive=False)
        ...
        def get_tags(items, tag, negative=False):
            ...
            if negative is True:
                return [
                    ...
                    if not (
                        e.name == tag
                        or e.parent.name == tag
                        or e.parent.parent.name == tag
                    )
                ]
            else:
                return [
        ...
        def grab_td(elements):
            ...
            for table in td_parents:
                ...
                self.result.append(
                    [
                        self.sanitize_string(tr.find_all("td")[highestIndex].text)
                        for tr in trs
                    ]
                )
        ...
        def grab_other(elements):
            ...
            for p in ep_pars_uniq:
                ...
                children = p.find_all(tag, recursive=False)
                if len(children) < 1:
        ...
        def compare_result(result):
            ...
            for res in result:
                ...
                if (
                    exists_count > 0
                    and percent > self.sensetivePercent
                    and percent < 100
                ):
        ...
        [
            ...
            for ignore_tag in soup.find_all(
                ["h1", "h2", "h3", "h4", "h5", "style", "script"]
            )
        ]
